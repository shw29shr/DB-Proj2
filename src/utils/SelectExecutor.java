...
public class SelectExecutor {
	...
	/**
	 * Select Executor constructor initialization. 
	 * Parses the input query statements one by one,
	 * builds the operator tree and evaluates
	 * @param statement
	...
	/**
	 * Return the table object according to its position in the query FROM clause
	 * @param tableIndex
	 *            	Index in the FROM clause
	 * @return Table object
	 */
	 ...
	 *
	 * @param tables
	 *            The tables present in the FROM clause
	...
	/**
	 * Given the position of a table,
	 * return the select condition associated with it
	 *
	 * @param pos
	 *            Position/index
	...
	/**
	 * Given the position of a table,
	 * return the join condition associated with it
	 *
	 * @param pos
	 *            Position/index
	 ...
	 * 
	 * @param currentRoot
	 *            Current temp root in the parse tree so far
	 * @return
	 */
	private Operator processAllSelectAndJoins(Operator currentRoot) {
		while (i < tableList.size()) {
			...
			currentRoot = new JoinOperator(currentRoot, getJoinCond(i), scanOp);
			i++;
	...
	/**
	 * Method which implements the core logic of the SQL interpreter
	 * Builds the operator tree in a bottom up fashion
	...
	private void generateTree() {
		...
		Operator currentRoot = getSelectCondition(0) != null
				? new SelectOperator((ScanOperator) new ScanOperator(getTableObject(0)), getSelectCondition(0))
				: new ScanOperator(getTableObject(0));
		...
		if (orderElements != null) {
			if (orderAllSelectedColumns) {
				currentRoot = new ProjectOperator(selectElements, new SortOperator(orderElements, currentRoot));
			} else {
				currentRoot = new SortOperator(orderElements, new ProjectOperator(selectElements, currentRoot));
			}
		} else // no order-by so just select all columns
			currentRoot = new ProjectOperator(selectElements, currentRoot);
		...
		// We completed parsing the tree, bottom-up, so we return
		root = currentRoot;
	 ...
	 * @param fromItem
	 * 				List of fromItems from the query
	 */
	 ...
	 * @param fromItem
	 * 				The first item in the FROM clause
	 */
	 ...
	 * We also add all the joinConditions
	 * @param joins
	 * 			List of Joins returned by the JSQLParser 
	 */
	 ...
	 * 
	 * @param tabNameOrAlias
	 *            Table name / its alias
	 * 
	 */
	...
	private void clearArraysForNextIteration() {
		...
		// Since we have one table atleast, we check for other tables.. This is an index into the array holding all tables
		i = 1; 
	}
